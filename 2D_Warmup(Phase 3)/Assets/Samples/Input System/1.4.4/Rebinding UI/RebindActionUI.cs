...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Disable the action before use, so that we can change the keybind in game as well. Because we have to disable it to do it in game.
            action.Disable();

            // Configure the rebind.
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                .WithControlsExcluding("<Mouse>/leftButton") //Excluding=loai tru
                ...
                .OnComplete(
                    operation =>
                    {
                        action.Enable(); //this was missing the last time no wonder it didn't work
                        m_RebindOverlay?.SetActive(false);
                        m_RebindStopEvent?.Invoke(this, operation);
                        
                        if(CheckDuplicateBindings(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex); //if it is a duplicate->return true->remove...
                            CleanUp();
        ...
        {
            InputBinding newBinding = action.bindings[bindingIndex]; //pathway to the key
            foreach(InputBinding binding in action.actionMap.bindings)
            {
                ...
                if(binding.effectivePath == newBinding.effectivePath)//to check if the key we press is the same
                {
                    Debug.Log("Duplicate finding founds: " + newBinding.effectivePath);
                    return true;
                }
            }
            ...
            return false;//we didn't find any duplicate bindings.
        }
